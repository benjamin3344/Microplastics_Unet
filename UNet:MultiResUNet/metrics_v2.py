...
def iou(actual, predicted):
    ...
    # predicted = K.flatten(predicted) > 0.45
    # predicted = tf.cast(predicted, dtype=float)
    ...
    # tf.print("actual*****:", actual, union, intersection, K.sum(actual), K.sum(predicted) )
...
def dice_coef(actual, predicted, eps=1e-3):
    ...
    # y_pred_f = K.flatten(predicted) > 0.45
    # y_pred_f = tf.cast(y_pred_f, dtype=float)
